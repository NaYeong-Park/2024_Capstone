...
from PyQt5.QtGui import QImage, QPixmap, QStandardItemModel, QStandardItem, QCursor, QPainter, QPen  # QPen 추가
...
class AugmentationApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        _ui = os.path.join(os.path.dirname(__file__), "picture UI.ui")
        uic.loadUi(_ui, self)
        ...
        self.is_cropping = False  # 크롭 모드 활성화 플래그
        self.selected_point = None  # 선택된 포인트를 저장할 변수

        # 버튼 클릭 이벤트 연결
        self.crop_button.clicked.connect(
            self.toggle_crop_mode)  # crop_button 클릭 연결

        # mouse event로 cropping 영역 설정
        self.graphicsView.mousePressEvent = self.onMousePress
        self.graphicsView.mouseReleaseEvent = self.onMouseRelease

    def toggle_crop_mode(self):  ## crop 을 위해 추가된 코드
        """Crop 모드 활성화 또는 비활성화"""
        self.is_cropping = not self.is_cropping  # Crop 모드를 토글함
        if self.is_cropping:
            self.add_log("Crop 모드 활성화")
            self.graphicsView.setCursor(Qt.CrossCursor)  # 마우스 커서를 교차 모양으로 변경
        else:
            self.add_log("Crop 모드 비활성화")
            self.graphicsView.setCursor(Qt.ArrowCursor)  # 마우스 커서를 기본 상태로 복원

    def onMousePress(self, event): ## crop 을 위해 추가된 코드
        """마우스 클릭 시 실행되는 함수"""
        scenePoint = self.graphicsView.mapToScene(event.pos())
        self.add_log(f"좌표: {scenePoint.x()}, {scenePoint.y()} ")
        
        if self.is_cropping:  # 크롭 모드가 활성화된 경우
            self.selected_point = (int(scenePoint.x()), int(scenePoint.y()))  # 클릭된 좌표 저장
            self.add_log(f"크롭을 위한 좌표 선택됨: {self.selected_point}")
            return  # 크롭 모드일 때는 다른 모드 무시
        ...
        if event.button() == Qt.LeftButton:
            if self.mode_free_roi:
                ...
                self.add_log("스티커 모드가 활성화 되었습니다.")
    ...
    def onMouseRelease(self, event):
        """마우스 클릭 후 크롭 영역 생성"""
        if self.is_cropping and self.selected_point:
            self.crop_image(self.selected_point)

    def crop_image(self, center_point):  ## crop 을 위해 추가된 코드
        if self.modified_image is None:
            self.add_log("Error: 이미지가 없습니다.")
            return

        x_center, y_center = center_point
        box_w, box_h = 300, 300  # 전체 크롭 영역 크기를 300x300으로 설정

        x_start = max(0, x_center - box_w // 2)
        y_start = max(0, y_center - box_h // 2)
        x_end = min(self.modified_image.shape[1], x_start + box_w)
        y_end = min(self.modified_image.shape[0], y_start + box_h)

        cropped_image = self.modified_image[y_start:y_end, x_start:x_end]
        self.add_log(f"이미지 크롭 완료: x({x_start}, {x_end}), y({y_start}, {y_end})")

        self.create_nine_images(center_point)
    ...
    def toggle_crop_mode(self): ## crop 을 위해 추가된 코드
        ...
        if self.is_cropping:
            self.add_log("Crop 모드 활성화")
            self.graphicsView.setCursor(Qt.CrossCursor)  # 마우스 커서를 교차 모양으로 변경
            self.deactivate_all_buttons()  # 다른 모드를 비활성화하여 충돌 방지
        else:
            self.add_log("Crop 모드 비활성화")
            self.graphicsView.setCursor(Qt.ArrowCursor)  # 마우스 커서를 기본 상태로 복원
    ...
    def load_image(self):
        ...
        self.image_path, _ = QFileDialog.getOpenFileName(
            self, "사진 선택", "", "Image Files (*.png *.jpg *.jpeg *.bmp)")
    ...
    def create_nine_images(self, center_point): ## crop 을 위해 추가된 코드
        x_center, y_center = center_point  # 클릭한 좌표를 중심으로 설정
        box_w, box_h = 100, 100  # 각 이미지를 100x100 크기로 설정

        nine_images = []
        shift = 20  # 각 이미지 간의 이동 간격 (20 픽셀씩 이동)

        # 3x3 그리드의 각 이미지를 생성
        for i in range(3):
            for j in range(3):
                # 중앙 이미지(클릭한 좌표)로부터 상하좌우로 약간씩 이동
                x_shift = (j - 1) * shift  # 왼쪽/오른쪽 이동 (j = 0, 1, 2)
                y_shift = (i - 1) * shift  # 위/아래 이동 (i = 0, 1, 2)

                # 이미지 영역을 계산
                x_start = max(0, x_center + x_shift - box_w // 2)
                y_start = max(0, y_center + y_shift - box_h // 2)
                x_end = min(self.modified_image.shape[1], x_start + box_w)
                y_end = min(self.modified_image.shape[0], y_start + box_h)

                cropped_image = self.modified_image[y_start:y_end, x_start:x_end]
                nine_images.append(cropped_image)

        self.display_nine_images(nine_images)

    def display_nine_images(self, images): ## crop 을 위해 추가된 코드
        """9개의 이미지를 3x3 그리드로 개별적으로 표시합니다."""
        self.scene2.clear()  # 기존 이미지를 지우고 새로 그리기

        img_size = 100  # 각 이미지의 크기
        margin = 10  # 이미지 사이에 여백을 추가할 경우
        grid_size = img_size * 3 + margin * 2  # 3x3 그리드의 전체 크기

        # 각 이미지를 3x3 그리드로 배치
        for i, img in enumerate(images):
            row = i // 3
            col = i % 3
            img_resized = cv2.resize(img, (img_size, img_size))  # 이미지를 100x100 크기로 조정
            qImg = QImage(img_resized.data, img_resized.shape[1], img_resized.shape[0], img_resized.strides[0], QImage.Format_BGR888)
            pixmap = QPixmap.fromImage(qImg)
            pixmapItem = QGraphicsPixmapItem(pixmap)
            
            # 각 이미지의 위치를 설정 (행과 열에 따라 좌표 계산)
            pixmapItem.setPos(col * (img_size + margin), row * (img_size + margin))
            self.scene2.addItem(pixmapItem)  # scene2에 개별 이미지 추가

        # 오른쪽 View에 크롭된 이미지를 표시 (graphicsView2 사용)
        self.graphicsView2.setScene(self.scene2)
        self.graphicsView2.setSceneRect(0, 0, grid_size, grid_size)  # 전체 그리드 크기 설정
        self.graphicsView2.fitInView(self.graphicsView2.sceneRect(), Qt.KeepAspectRatio)
        self.add_log("9개의 이미지 생성 완료")
        


    def display_image(self, image):
        image = np.ascontiguousarray(image)  # 배열을 C 연속형으로 변환
        qImg = QImage(sip.voidptr(
            # BGR 포맷 사용
            image.data), image.shape[1], image.shape[0], image.strides[0], QImage.Format_BGR888)
        ...
        # 주어진 점을 감싸는 최소 크기 사각형(바운딩박스)를 반환
        self.graphicsView.setSceneRect(pixmapItem.boundingRect())
        self.graphicsView2.fitInView(
            pixmapItem, Qt.KeepAspectRatio)  # 이미지 크기를 View에 맞추어 조정
    ...
    def display_free_image(self, image):
        qImg = QImage(
            image.data, image.shape[1], image.shape[0], image.strides[0], QImage.Format_RGBA8888)
        pixmap = QPixmap.fromImage(qImg)
        pixmapItem = QGraphicsPixmapItem(pixmap)
        self.scene2.clear()
        self.scene2.addItem(pixmapItem)
    ...
    def onMousePress(self, event):
        scenePoint = self.graphicsView.mapToScene(event.pos())
        self.add_log(f"좌표: {scenePoint.x()}, {scenePoint.y()} ")
        if self.is_cropping:           ## crop 을 위해 추가된 코드
            scenePoint = self.graphicsView.mapToScene(event.pos())
            self.crop_image((int(scenePoint.x()), int(scenePoint.y())))
            return
        if event.button() == Qt.LeftButton:
            if self.mode_free_roi:
                self.roi_path = QtGui.QPainterPath()
                ...
                if not self.selectionRect:
                    self.selectionRect = self.scene.addRect(
                        self.x0, self.y0, 0, 0, pen=QtGui.QPen(Qt.red, 2))
                else:
                    self.selectionRect.setRect(self.x0, self.y0, 0, 0)
            elif self.sticker_mode:
    ...
    def onMouseRelease(self, event):
        if self.mode_free_roi and self.roi_path_item and self.roi_path:
            ...
            cropped_image = self.modified_image[int(
            ...
            mask_image = QImage(
                mask.shape[1], mask.shape[0], QImage.Format_Grayscale8)
            mask_image.fill(0)
            ...
            mask = np.array(ptr, dtype=np.uint8).reshape(
            ...
            if w > 0 and h > 0:
                if self.is_blur_selected:
                    ...
                    self.modified2_image = self.modified_image[int(self.y0):int(
                        self.y0 + h), int(self.x0):int(self.x0 + w)]
            ...
            if self.selectionRect:
                ...
                self.add_log('ROI가 선택되었습니다. - x:%d, y:%d, w:%d, h:%d' %
                             (self.x0, self.y0, w, h))
                self.lineEdit_roi.setText(
                    str('x:%d, y:%d, w:%d, h:%d' % (self.x0, self.y0, w, h)))
    ...
    def place_sticker(self, position):
        ...
        if sticker_image is not None:
            ...
            pixmap = QPixmap.fromImage(qImg)
            pixmapItem = QGraphicsPixmapItem(pixmap)
            pixmapItem.setPos(position.x() - pixmap.width() / 2,
                              position.y() - pixmap.height() / 2)
            self.scene.addItem(pixmapItem)
            self.add_log("스티커가 중심 좌표 : ({}, {})에 배치되었습니다.".format(
    ...
    def calculate_distance(self, point):
        ...
        if len(self.points) == 2:  # 수정: self.points1을 self.points로 변경
            ...
            self.point_lineEdit.setText(
                f"({p1[0]}, {p1[1]}) and ({p2[0]}, {p2[1]})")
            self.point2_textEdit.setText(f"총 거리: {distance:.2f}\nx축 거리: {
                                         x_distance:.2f}\nY축 거리: {y_distance:.2f}")

            self.add_log(f"거리 계산됨: 총 거리 {distance:.2f}, X축 {
                         x_distance:.2f}, Y축 {y_distance:.2f}")
    ...
    def update_button_styles(self):
        ...
        if self.mode_free_roi:
            self.free_roi_button.setStyleSheet(
                "QPushButton { background-color: green; color: black; }")
        else:
            self.free_roi_button.setStyleSheet("")
    ...
    def cv_image_to_qpixmap(self, cv_image):
        ...
        q_image = QImage(cv_image.tobytes(), width, height,
...
###########################  사진증강  #########################################################################################################################################################
    ...
    def adjust_brightness(self):
        try:
            ...
            if self.modified_image is None:
                ...
                return
            self.modified2_image = cv2.convertScaleAbs(
                self.original_image, alpha=factor, beta=factor)
    ...
    def rotate_image(self):
        try:
            ...
            # 이미지 중심을 계산
            image_center = tuple(
                np.array(self.modified_image.shape[1::-1]) / 2)
            ...
            bound_w = int(
                self.modified_image.shape[1] * abs_cos + self.modified_image.shape[0] * abs_sin)
            bound_h = int(
                self.modified_image.shape[1] * abs_sin + self.modified_image.shape[0] * abs_cos)
            ...
            rotated_image = cv2.warpAffine(
                self.modified_image, rot_mat, (bound_w, bound_h), flags=cv2.INTER_LINEAR)
    ...
    def resize_image(self):
        ...
        # 새로운 이미지 차원 계산
        new_dimensions = (int(self.original_image.shape[1] * scale_factor), int(
            self.original_image.shape[0] * scale_factor))
        self.modified2_image = cv2.resize(
            self.original_image, new_dimensions, interpolation=cv2.INTER_LINEAR)
        self.display_image2(self.modified2_image)
    ...
    def show_blur(self):
        ...
        try:
            ...
            qImg = QImage(
                final_image.data, final_image.shape[1], final_image.shape[0], final_image.strides[0], QImage.Format_RGB888)
            ...
            self.add_log(
                "Blur applied to selected area with intensity: {}".format(blur_amount))
...
#######################    roi 증강  ##################################################################################################################################################################
    ...
    def roi_adjust_brightness(self):
        try:
            ...
            if self.modified2_image is None:
                ...
                return
            self.modified2_image = cv2.convertScaleAbs(
                self.modified2_image, alpha=factor, beta=factor)
    ...
    def roi_rotate_image(self):
        try:
            ...
            rotated_image = cv2.warpAffine(self.modified2_image, rot_mat,
                                           (self.modified2_image.shape[1],
                                            self.modified2_image.shape[0]),
    ...
    def roi_resize_image(self):
        ...
        # 새로운 이미지 차원 계산
        new_dimensions2 = (int(self.modified2_image.shape[1] * scale_factor), int(
            self.modified2_image.shape[0] * scale_factor))
        self.modified2_image = cv2.resize(
            self.modified2_image, new_dimensions2, interpolation=cv2.INTER_LINEAR)
       # self.modified2_image = cv2.cvtColor(self.modified2_image, cv2.COLOR_BGR2RGB)
        self.display_image3(self.modified2_image)
        ...
        if self.modified2_image is not None:
            ...
            if save_path:  # 사용자가 경로를 선택한 경우에만 이미지를 저장
                cv2.imwrite(save_path, cv2.cvtColor(
                    self.modified2_image, cv2.COLOR_RGB2BGR))  # OpenCV를 사용하여 이미지를 저장
                self.add_log("이미지가 성공적으로 저장되었습니다.")
        else:
            self.add_log("저장할 이미지가 없습니다.")
    ...
    def gray_change(self, checked):
        if checked:
            gray_image = cv2.cvtColor(
                self.modified_image, cv2.COLOR_BGR2GRAY)  # 원본 이미지를 회색조로 변환
            # OpenCV의 회색조 이미지를 RGB 이미지로 변환 (QPixmap에 맞게 조정)
            gray_image = cv2.cvtColor(gray_image, cv2.COLOR_GRAY2RGB)
            qImg = QImage(gray_image.data, gray_image.shape[1], gray_image.shape[0],
                          # QImage 객체 생성
                          gray_image.strides[0], QImage.Format_RGB888)
            gray_pixmap = QPixmap.fromImage(qImg)  # QPixmap으로 변환
            self.scene2.clear()
            # QGraphicsScene에 QPixmap 객체 추가
            pixmapItem = QGraphicsPixmapItem(gray_pixmap)
            self.scene2.addItem(pixmapItem)
            self.graphicsView2.setScene(self.scene2)
            self.graphicsView2.setSceneRect(pixmapItem.boundingRect())
            self.add_log("Gray Image 생성 완료")
        else:
            # 회색조 변경이 아닌 경우 원래 이미지로 되돌림
            self.display_image(self.modified_image)
    ...
    def thres_change(self):
        ...
        try:
            ...
            if not (0 <= threshold_value <= 255):
                self.add_log(
                    "Error-Threshold value must be between 0 and 255.")
                return
            ...
            binary_image = cv2.cvtColor(
                binary_image, cv2.COLOR_GRAY2RGB)  # 이진화된 이미지를 RGB 이미지로 변환
            ...
            self.scene2.clear()
            # QGraphicsScene에 QPixmap 객체 추가
            pixmapItem = QGraphicsPixmapItem(binary_pixmap)
            self.scene2.addItem(pixmapItem)
            self.graphicsView2.setScene(self.scene2)
            self.graphicsView2.setSceneRect(pixmapItem.boundingRect())
            ...
            self.add_log(
                "Error-Invalid input. Please enter a valid integer for threshold.")
    ...
    def canny_change(self):
        ...
        try:
            ...
            canny_image = cv2.Canny(
                gray_image, canny_value, canny_value * 2)  # Canny 함수로 에지 검출
            # 에지 검출된 이미지를 RGB 이미지로 변환
            canny_image = cv2.cvtColor(canny_image, cv2.COLOR_GRAY2RGB)
            qImg = QImage(
                canny_image.data, canny_image.shape[1], canny_image.shape[0], canny_image.strides[0], QImage.Format_RGB888)
            canny_pixmap = QPixmap.fromImage(qImg)
            self.scene2.clear()
            # QGraphicsScene에 QPixmap 객체 추가
            pixmapItem = QGraphicsPixmapItem(canny_pixmap)
            self.scene2.addItem(pixmapItem)
            self.graphicsView2.setScene(self.scene2)
            self.graphicsView2.setSceneRect(pixmapItem.boundingRect())
    ...
    def morph_change(self):
        ...
        try:
            ...
            _, binary_image = cv2.threshold(
                gray_image, morph_value, 255, cv2.THRESH_BINARY)

            # 모폴로지 연산 적용: 여기서는 예로 침식과 팽창을 사용
            kernel = cv2.getStructuringElement(
                cv2.MORPH_RECT, (5, 5))  # 5x5 크기의 사각형 커널
            eroded_image = cv2.erode(
                binary_image, kernel, iterations=1)  # 침식 연산
            dilated_image = cv2.dilate(
                eroded_image, kernel, iterations=1)  # 팽창 연산

            # 결과 이미지를 RGB로 변환
            result_image = cv2.cvtColor(dilated_image, cv2.COLOR_GRAY2RGB)
            qImg = QImage(
                result_image.data, result_image.shape[1], result_image.shape[0], result_image.strides[0], QImage.Format_RGB888)
    ...
    def blur_change(self):
        ...
        try:
            ...
            blurred_image = cv2.GaussianBlur(
                gray_image, (blur_amount, blur_amount), 0)
            # 다시 RGB로 변환하여 QGraphicsView에서 표시
            blurred_image = cv2.cvtColor(blurred_image, cv2.COLOR_GRAY2RGB)

            qImg = QImage(
                blurred_image.data, blurred_image.shape[1], blurred_image.shape[0], blurred_image.strides[0], QImage.Format_RGB888)
            blur_pixmap = QPixmap.fromImage(qImg)
            self.scene2.clear()
            # QGraphicsScene에 QPixmap 객체 추가
            pixmapItem = QGraphicsPixmapItem(blur_pixmap)
    ...
    def shar_change(self):
        ...
        try:
            ...
            if not (0 <= shar_amount <= 255):
                self.add_log(
                    "Error-Sharpness amount must be between 0 and 255.")
                return
            ...
            # RGB로 변환하여 QGraphicsView에서 표시
            sharpened_image = cv2.cvtColor(sharpened_image, cv2.COLOR_GRAY2RGB)

            # 이미지 표시를 위한 QImage 객체 생성
            qImg = QImage(
                sharpened_image.data, sharpened_image.shape[1], sharpened_image.shape[0], sharpened_image.strides[0], QImage.Format_RGB888)
            shar_pixmap = QPixmap.fromImage(qImg)
            self.scene2.clear()
            # QGraphicsScene에 QPixmap 객체 추가
            pixmapItem = QGraphicsPixmapItem(shar_pixmap)
            self.scene2.addItem(pixmapItem)
            self.graphicsView2.setScene(self.scene2)
            self.graphicsView2.setSceneRect(pixmapItem.boundingRect())
            self.add_log(
                "Sharpness applied with intensity: {}".format(shar_amount))
        except ValueError:
            self.add_log(
                "Error-Invalid input. Please enter a valid integer for sharpness.")
